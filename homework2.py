...
def process_frame(frame, template_list):
    ...
    if best_match:
        ...
        print("score:", max_corr, ", best_template:", best_template, ", (x, y):", best_match)
        x, y = best_match
        last_x, last_y = best_match

        # 绘制目标的椭圆形框
        # cv2.rectangle(frame, (x, y), (x + best_template_width, y + best_template_height), (0, 255, 0), 2) 
        center_coordinates = (x + best_template_width // 2, y + best_template_height // 2)
        axes_lengths = (best_template_width // 2, best_template_height // 2)
        cv2.ellipse(frame, center_coordinates, axes_lengths, 0, 0, 360, (255, 255, 0), 2)
...
if __name__ == "__main__":
    ...
    template1 = cv2.imread("template1.png")  # 读取RGB模板
    ...
    # 开始目标跟踪
    track_target(video_path, template, "output2.mp4")
